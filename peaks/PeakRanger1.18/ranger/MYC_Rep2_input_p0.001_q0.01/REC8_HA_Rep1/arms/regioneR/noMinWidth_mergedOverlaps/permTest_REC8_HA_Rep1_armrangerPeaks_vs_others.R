...
# csmit -m 50G -c 48 "Rscript permTest_REC8_HA_Rep1_armrangerPeaks_vs_others.R"
...
REC8_MYC_Rep1GR <- armrangerPeaksGRmergedOverlaps
armrangerPeaksGRmergedOverlaps <- NULL
...
load(paste0(inDir,
            "REC8_MYC_Rep2_armrangerPeaksGRmergedOverlaps_minuslog10_p0.001_q0.01_noMinWidth.RData"))
REC8_MYC_Rep2GR <- armrangerPeaksGRmergedOverlaps
armrangerPeaksGRmergedOverlaps <- NULL
...
save(ptPeaksOtherPerChrom,
     file = paste0(outDir,
                   "permTest_REC8_HA_Rep1_armrangerPeaks_vs_others.RData"))
...
write.table(ptPeaksOtherPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_REC8_HA_Rep1_armrangerPeaks_vs_others_DataFrame.txt"),
            sep = "\t", row.names = F)
...
# Plot histograms of arm peak widths

histDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/REC8_HA_Rep1/arms/regioneR/noMinWidth_mergedOverlaps/hist/"

load(paste0(inDir,
            "REC8_HA_Rep1_armrangerPeaksGRmergedOverlaps_minuslog10_p0.001_q0.01_noMinWidth.RData"))
REC8_HA_Rep1GR <- armrangerPeaksGRmergedOverlaps
armrangerPeaksGRmergedOverlaps <- NULL
load(paste0(inDir,
            "REC8_MYC_Rep1_armrangerPeaksGRmergedOverlaps_minuslog10_p0.001_q0.01_noMinWidth.RData"))
REC8_MYC_Rep1GR <- armrangerPeaksGRmergedOverlaps
...
for(i in 1:length(grlHist)) {
  xname <- paste0(namesHist[i], " arm peak width (bp)")
  hist(width(grlHist[[i]]), breaks = 250, col = "grey60", border = NA, lwd = 2,
       xlab = paste0(namesHist[i], " arm peak width (bp)"), ylab = "Peaks", main = "", cex.lab = 1.25, cex.axis = 1.25)
