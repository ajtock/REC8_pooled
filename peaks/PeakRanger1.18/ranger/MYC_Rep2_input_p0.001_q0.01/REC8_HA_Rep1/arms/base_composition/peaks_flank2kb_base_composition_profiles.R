...
mask <- toGRanges(data.frame(rep(chrs, 3),
                             c(rep(1, 5), pericenStart, chrLens-2000),
                             c(rep(2000, 5), pericenEnd, chrLens)))

inDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/"
outDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/REC8_HA_Rep1/arms/base_composition/"
plotDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/REC8_HA_Rep1/arms/base_composition/plots/"

# Import peaks as GRanges object
load(paste0(inDir,
            "REC8_HA_Rep1_armrangerPeaksGRmergedOverlaps_minuslog10_p0.001_q0.01_noMinWidth.RData"))
targetsGR <- armrangerPeaksGRmergedOverlaps
print("***********peaks***********")
print(targetsGR)
# Generate GRanges object containing random loci of same number
## and size distribution as targetsGR
##ranLocGR <- randomizeRegions(targetsGR,
#                             genome = genome,
#                             mask = mask,
#                             per.chromosome = TRUE,
#                             allow.overlaps = TRUE)
...
baseRelativeFreqUnstranded(targets = targetMidpointsGR, genome = genome, mask = mask,
                           ...
                           outDir = outDir, targetName = "arm")

# Load base relative frequency files
target.baseFreq <- list(read.table(file = paste0(outDir, "arm_target.a.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_target.t.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_target.g.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_target.c.txt"))[,1])
ranLoc.baseFreq <- list(read.table(file = paste0(outDir, "arm_ranLoc.a.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_ranLoc.t.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_ranLoc.g.txt"))[,1],
                        read.table(file = paste0(outDir, "arm_ranLoc.c.txt"))[,1])
...
pdf(paste0(plotDir, "REC8_HA_Rep1_armpeak_base_relative_frequency_diffYaxes_v080618.pdf"), height = 5, width = 6)
par(mfrow = c(2, 2))
...
mergeBaseFreqPlotDiffY(at.coords = target.ATfreq, gc.coords = target.GCfreq,
                       ...
                       mainTitle1 = "Arm REC8-HA Rep1 peaks", mainTitle2 = "Arm random loci")
...
pdf(paste0(plotDir, "REC8_HA_Rep1_armpeak_base_relative_frequency_v080618.pdf"), height = 5, width = 6)
par(mfrow = c(2, 2))
par(mar = c(2.1, 3.2, 2.1, 3.2))
par(mgp = c(2.25, 1, 0))
flankSize <- 2000
...
pdf(paste0(plotDir, "REC8_HA_Rep1_armpeak_base_relative_frequency_AT_and_GC_diffYlims_v080618.pdf"), height = 10, width = 6)
par(mfrow = c(4, 2))
par(mar = c(2.1, 3.2, 2.1, 3.2))
par(mgp = c(2.25, 1, 0))
flankSize <- 2000
