...
# csmit -m 50G -c 24 "Rscript permTest_REC8_HA_Rep1_rangerPeaks_vs_TEfams.R"
...
DNAplotDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/REC8_HA_Rep1/genome_wide/regioneR/noMinWidth_mergedOverlaps/plots/TEsDNA/"
RNAplotDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/peaks/PeakRanger1.18/ranger/MYC_Rep2_input_p0.001_q0.01/REC8_HA_Rep1/genome_wide/regioneR/noMinWidth_mergedOverlaps/plots/TEsRNA/"
...
save(ptPeaksTEsDNAPerChrom,
     file = paste0(outDir,
                   "permTest_REC8_HA_Rep1_rangerPeaks_vs_TEsDNA.RData"))
...
write.table(ptPeaksTEsDNAPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_REC8_HA_Rep1_rangerPeaks_vs_TEsDNA_DataFrame.txt"),
            sep = "\t", row.names = F)

# plot graphical summaries of results
for(i in 1:length(ptPeaksTEsDNAPerChrom)) {
  pdf(file = paste0(DNAplotDir, DNAfamNames[i], "_permTest_nperm10000_REC8_HA_Rep1_rangerPeaks_perChrom.pdf"), width = 10, height = 7)
  plot(ptPeaksTEsDNAPerChrom[[i]], main = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  pdf(file = paste0(DNAplotDir, DNAfamNames[i], "_localZscore_permTest_nperm10000_REC8_HA_Rep1_rangerPeaks_w1kb_s50bp_w10kb_s500bp_w", win ,"kb_s", step, "bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", DNAfamNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
...
save(ptPeaksTEsRNAPerChrom,
     file = paste0(outDir,
                   "permTest_REC8_HA_Rep1_rangerPeaks_vs_TEsRNA.RData"))
...
write.table(ptPeaksTEsRNAPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_REC8_HA_Rep1_rangerPeaks_vs_TEsRNA_DataFrame.txt"),
            sep = "\t", row.names = F)

# plot graphical summaries of results
for(i in 1:length(ptPeaksTEsRNAPerChrom)) {
  pdf(file = paste0(RNAplotDir, RNAfamNames[i], "_permTest_nperm10000_REC8_HA_Rep1_rangerPeaks_perChrom.pdf"), width = 10, height = 7)
  plot(ptPeaksTEsRNAPerChrom[[i]], main = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  win <- as.character(round((10*mean(width(peaksGR)))/1000))
  step <- as.character(round(mean(width(peaksGR))/2))
  pdf(file = paste0(RNAplotDir, RNAfamNames[i], "_localZscore_permTest_nperm10000_REC8_HA_Rep1_rangerPeaks_w1kb_s50bp_w10kb_s500bp_w", win ,"kb_s", step, "bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("REC8_HA_Rep1_rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
