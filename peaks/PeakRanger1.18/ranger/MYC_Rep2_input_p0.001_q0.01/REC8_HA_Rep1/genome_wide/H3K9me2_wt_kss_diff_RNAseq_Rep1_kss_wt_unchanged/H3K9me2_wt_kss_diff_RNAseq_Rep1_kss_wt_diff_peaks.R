...
wt_norm <- read.table("/projects/ajt200/BAM_masters/H3K9me2/WT/coverage/log2ChIPinput/log2_WT_H3K9me2_ChIP_WT_H3K9me2_input_norm_allchrs_coverage_coord_tab.bed",
                      colClasses = c(NA, NA, "NULL", NA))
kss_norm <- read.table("/projects/ajt200/BAM_masters/H3K9me2/kss/coverage/log2ChIPinput/log2_kss_H3K9me2_ChIP_kss_H3K9me2_input_norm_allchrs_coverage_coord_tab.bed", 
                       colClasses = c(NA, NA, "NULL", NA))
wt_RNAseq_norm <- read.table("/home/meiosis/ajt200/analysis/170918_Chris_RNAseq_Col_kss/fastq_pooled/WT/coverage/WT_RNAseq_Chris_Rep1_norm_allchrs_coverage_coord_tab.bed",
                             colClasses = c(NA, NA, "NULL", NA))
kss_RNAseq_norm <- read.table("/home/meiosis/ajt200/analysis/170918_Chris_RNAseq_Col_kss/fastq_pooled/kss/coverage/kss_RNAseq_Chris_Rep1_norm_allchrs_coverage_coord_tab.bed",
                              colClasses = c(NA, NA, "NULL", NA))
...
for(i in 1:5) {
  ...
  print(i)
  wt_RNAseq_chrCov <- wt_RNAseq_norm[wt_RNAseq_norm[,1] == chrs[i],]
  wt_RNAseq_chrCov <- wt_RNAseq_chrCov[,3]
  kss_RNAseq_chrCov <- kss_RNAseq_norm[kss_RNAseq_norm[,1] == chrs[i],]
  kss_RNAseq_chrCov <- kss_RNAseq_chrCov[,3]
  print(i)
  ...
  print(i)
  wt_RNAseq_norm_peakCov <- sapply(peaksOverlaps,
                                   function(x) mean(wt_RNAseq_chrCov[x]))
  kss_RNAseq_norm_peakCov <- sapply(peaksOverlaps,
                                    function(x) mean(kss_RNAseq_chrCov[x]))
  print(i)
  chrPeaks <- cbind(rep(chrs[i], length(chrPeaksGR)),
                    ...
                    wt_RNAseq_norm_peakCov,
                    kss_RNAseq_norm_peakCov,
                    kss_RNAseq_norm_peakCov-wt_RNAseq_norm_peakCov)
...
colnames(allCovPeaks) <- c("Chr", "Start", "End",
                           ...
                           "wt_RNAseq_norm_peakCov", "kss_RNAseq_norm_peakCov", "diff_kss_wt_RNAseq_norm_peakCov")
