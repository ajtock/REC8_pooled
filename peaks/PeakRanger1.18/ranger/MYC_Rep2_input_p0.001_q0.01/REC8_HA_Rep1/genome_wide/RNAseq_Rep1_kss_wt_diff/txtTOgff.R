...
peaks <- read.table(paste0(inDir,
                           libName, "_peaks_mean_", diffName, "_cov_top10percent.txt"),
                    header = T)

peaks_sorted <- peaks[order(-peaks$diff_kss_wt_norm_peakCov),]
write.table(peaks_sorted,
            file = paste0(inDir,
                          libName, "_peaks_mean_", diffName, "_cov_top10percent_sorted.txt"),
            sep = "\t", quote = F)
...
peaks_sorted_gff <- cbind(peaks_sorted$Chr,
                          ...
                          peaks_sorted$Start,
                          peaks_sorted$End,
                          peaks_sorted$diff_kss_wt_norm_peakCov,
                          rep("."),
                          rep("."),
                          rep("."))
...
write.table(peaks_sorted_gff,
            file = paste0(inDir,
                          libName, "_peaks_mean_", diffName, "_cov_top10percent_sorted.gff"),
