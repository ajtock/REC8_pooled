...
# Usage:
# ./other_features_wt_only_REC8_SPO11oligos_nucleosomes_arm_peaks.R "Arm REC8-HA peaks, nucleosomes and SPO11-1-oligo hotspots" "REC8-HA" "SPO11-1" "Nucleosomes" REC8_HA_Rep2 wt_SPO11oligo nucleosomes REC8_HA_Rep2 arm 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Arm REC8-HA peaks, nucleosomes and SPO11-1-oligo hotspots"
#pt1Name <- "REC8-HA"
#pt2Name <- "SPO11-1"
#pt3Name <- "Nucleosomes"
...
ptOrder <- ptOrder[c(1, 3:length(ptOrder))]
...
pt1 <- pt1[c(1, 3:length(pt1))]
ptPeaksOtherPerChrom <- NULL

# Re-order pt2 and pt3 to make consistent with pt1
# REC8-Myc Rep1 peaks, well-positioned nucleosomes, REC8-HA Rep2 peaks
load(paste0(pt2Dir, "permTest_", perms, "perms_",
            pt2LibName, "_hotspots_", region, "_vs_others.RData"))
pt2 <- ptPeaksOtherPerChrom
pt2 <- pt2[c(3, 2, 1, 4:length(pt2))]
ptPeaksOtherPerChrom <- NULL

# REC8-Myc Rep1 peaks, REC8-HA Rep2 peaks, SPO11-1-oligo hotspots
load(paste0(pt3Dir, "permTest_", perms, "perms_",
            pt3LibName, "_", region, "_vs_others.RData"))
pt3 <- ptPeaksOtherPerChrom
pt3 <- pt3[c(3, 1, 2, 4:length(pt3))]
ptPeaksOtherPerChrom <- NULL
...
bp <- ggplot(data = df,
  ...
  scale_y_continuous(limits = c(-5.00, 2.50)) +
