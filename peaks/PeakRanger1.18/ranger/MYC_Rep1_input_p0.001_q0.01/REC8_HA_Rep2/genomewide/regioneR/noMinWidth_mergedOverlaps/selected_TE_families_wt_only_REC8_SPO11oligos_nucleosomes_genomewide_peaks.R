...
# Usage:
# ./selected_TE_families_wt_only_REC8_SPO11oligos_nucleosomes_genomewide_peaks.R "Genome-wide REC8-HA peaks, nucleosomes and SPO11-1-oligo hotspots" "REC8-HA" "SPO11-1" "Nucleosomes" REC8_HA_Rep2 wt_SPO11oligo nucleosomes nucleosomes genomewide 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Genome-wide REC8-HA peaks, nucleosomes and SPO11-1-oligo hotspots"
#pt1Name <- "REC8-HA"
#pt2Name <- "SPO11-1"
#pt3Name <- "Nucleosomes"
...
ptOrderDir <- paste0("/projects/ajt200/REC8_MSH4/nuc_peaks/log2ChIPinput/nucleR/trim/analysis_01/",
                     "/genomewide/regioneR/mergedOverlaps/")
...
famNumbers <- c(1:5, 8:11)
famNames <- c("dna", "heli", "ptmari", "mudr", "enspm", "hat", "harbinger",
              "rna", "gypsy", "copia", "linel1", "sine")
famNames <- famNames[famNumbers]
famNamesPlot <- c("DNA", "Helitron", "Pogo/Tc1/Mariner", "MuDR", "EnSpm/CACTA", "hAT", "Harbinger",
                  "RNA", "Gypsy LTR", "Copia LTR", "LINE-1", "SINE")
famNamesPlot <- famNamesPlot[famNumbers]
...
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_", region, "_peaks_vs_TEsDNA.RData"))
ptOrder_DNA <- ptPeaksTEsDNAPerChrom
ptPeaksTEsDNAPerChrom <- NULL
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_", region, "_peaks_vs_TEsRNA.RData"))
...
# Combined
ptOrder <- c(ptOrder_DNA, ptOrder_RNA)[famNumbers]
pt1 <- c(pt1_DNA, pt1_RNA)[famNumbers]
pt2 <- c(pt2_DNA, pt2_RNA)[famNumbers]
pt3 <- c(pt3_DNA, pt3_RNA)[famNumbers]
...
bp <- ggplot(data = df,
  ...
  theme(axis.line.y = element_line(size = 1, colour = "black"),
        ...
        axis.text.x = element_text(size = 20, colour = "black", hjust = 0, vjust = 0.5, angle = 45),
  ...
  ggtitle(paste0(plotTitle, " (", prettyNum(as.character(perms),
                                            big.mark = ",", trim = "T"),
                 " permutations)"))
ggsave(paste0(plotDir, "barplot_selected_TE_families_permTestResults_",
       ...
       height = 8, width = 12)
save(bp,
     file = paste0(plotDir, "barplot_selected_TE_families_permTestResults_",
                   as.character(perms), "perms_",
                   "log2_Observed_Expected_",
                   pt1LibName, "_", pt2LibName, "_", pt3LibName, "_",
                   region, "_peaks.RData"))
