...
# Usage: 
# /applications/R/R-3.5.0/bin/Rscript chi_square_test_arm_peaks_vs_others.R REC8_MYC_Rep1_ChIP_rangerPeaksGR_arm_mergedOverlaps_noMinWidth.RData kss_REC8_HA_Rep1_ChIP_rangerPeaksGR_arm_mergedOverlaps_noMinWidth.RData "wt REC8-Myc" "kss REC8-HA" "wt SPO11-1" "kss SPO11-1" REC8_MYC_Rep1 kss_REC8_HA_Rep1 wt_SPO11oligo kss_SPO11oligo "Arm REC8-Myc peaks and SPO11-1-oligo hotspots" arm

#peakFile1 <- "REC8_MYC_Rep1_ChIP_rangerPeaksGR_arm_mergedOverlaps_noMinWidth.RData"
#peakFile2 <- "kss_REC8_HA_Rep1_ChIP_rangerPeaksGR_arm_mergedOverlaps_noMinWidth.RData"
#peakName1 <- "wt REC8-Myc"
#peakName2 <- "kss REC8-HA"
#peakName3 <- "wt SPO11-1"
#peakName4 <- "kss SPO11-1"
#peakLibName1 <- "REC8_MYC_Rep1"
#peakLibName2 <- "kss_REC8_HA_Rep1"
#peakLibName3 <- "wt_SPO11oligo"
#peakLibName4 <- "kss_SPO11oligo"
#plotTitle <- "Arm REC8-Myc peaks and SPO11-1-oligo hotspots"
#region <- "arm"
...
load(paste0(inDir,
            peakFile1))
wtREC8GR <- rangerPeaksGR_arm_mergedOverlaps
rangerPeaksGR_arm_mergedOverlaps <- NULL
strand(wtREC8GR) <- "*"
print("***********wt_REC8_MYC_Rep1_peaks***********")
print(wtREC8GR)
print(length(wtREC8GR))

# kss REC8 peaks
load(paste0("/home/ajt200/analysis/180622_Chris_lambing_ChIP_REC8_HA_Col_kss/kss/peaks/PeakRanger1.18/ranger/rep_specific_input_p0.001_q0.01/",
            peakFile2))
kssREC8GR <- rangerPeaksGR_arm_mergedOverlaps
rangerPeaksGR_arm_mergedOverlaps <- NULL
strand(kssREC8GR) <- "*"
...
wtSPO11GR <- armrangerPeaksGRmerge
...
kssSPO11GR <- armrangerPeaksGRmerge
armrangerPeaksGRmerge <- NULL
strand(kssSPO11GR) <- "*"
...
chisqPvalsSPO11 <- sapply(1:length(grl), function(x) {
  chisq.test(x = contingencyTable(wtPeaksIn = wtSPO11overlaps[x],
                                  ...
                                  kssPeaksOut = length(kssSPO11GR)-kssSPO11overlaps[x]),
...
bp <- ggplot(data = df,
  ...
  annotate(geom = "text",
           x = c(0.8, 1.2,
                 ...
                 5.8, 6.2),
           y = c(80), angle = 90,
           label = c(paste0("P = ", chisqPvalsREC8[1]),
                     paste0("P = ", chisqPvalsSPO11[1]),
                     paste0("P = ", chisqPvalsREC8[2]),
                     paste0("P = ", chisqPvalsSPO11[2]), 
                     paste0("P = ", chisqPvalsREC8[3]),
                     paste0("P = ", chisqPvalsSPO11[3]),
                     paste0("P = ", chisqPvalsREC8[4]),
                     paste0("P = ", chisqPvalsSPO11[4]),
                     paste0("P = ", chisqPvalsREC8[5]),
                     paste0("P = ", chisqPvalsSPO11[5]),
                     paste0("P = ", chisqPvalsREC8[6]),
                     paste0("P = ", chisqPvalsSPO11[6])))
