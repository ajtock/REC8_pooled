...
# Well-positioned nucleosomes [2]
# SPO11-1-oligo hotspots/REC8-Myc peaks [3]
# H3K27me3 peaks [11]
# H3K9me2 peaks [12]
# Crossovers [13]

# Usage:
# ./other_selected_features_wt_only_REC8_SPO11oligos_peri_peaks.R "Pericentromeric REC8-Myc peaks and SPO11-1-oligo hotspots" "wt REC8-Myc" "wt SPO11-1" REC8_MYC_Rep1 wt_SPO11oligo REC8_HA_Rep2 peri 10000
...
bp <- ggplot(data = df,
  ...
  geom_point(mapping = aes(Feature, log2alpha0.05),
             ...
             shape = "-", colour  = "grey70", size = 8) +
  labs(y = expression("Log"[2]*"(observed/expected) peak overlap")) +
  scale_y_continuous(limits = c(-3.1, 1)) +
  scale_x_discrete(position = "top") +
  guides(fill = guide_legend(direction = "horizontal",
                             label.position = "top",
                             label.theme = element_text(size = 8, hjust = 0, vjust = 0.5, angle = 90),
                             nrow = 1,
                             byrow = TRUE)) +
  theme_bw() +
  theme(axis.line.y = element_line(size = 0.5, colour = "black"),
        axis.ticks.y = element_line(size = 0.5, colour = "black"),
        axis.text.y = element_text(size = 8, colour = "black", hjust = 0.5, vjust = 0.5, angle = 90),
        axis.title.y = element_text(size = 8, colour = "black"),
        axis.ticks.x = element_blank(),
        axis.text.x = element_text(size = 8, colour = "black", hjust = 0, vjust = 0.5, angle = 90),
        ...
        plot.title = element_text(size = 7, colour = "black", hjust = 0.25)) +
