...
plotDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/coverage/common_input_MYC_Rep2/log2ChIPinput/geneProfiles/geneHexiles/WT_RNAseq_meiocyte_Rep1/analysis_01/plots/"
...
outDFCM <- lapply(seq_along(libNames), function(x)
  list(paste0(matDir, libNames[[x]],
              "_norm_cov_gene_hexile", c(1:6), "_by_WT_RNAseq_meiocyte_Rep1_mat1_smoothed_target_and_flank_dataframe_colMeans.txt"),
...
pdf(paste0(plotDir, "RNAseq_meiocyte_Rep1_geneHexileProfiles_winSize20.pdf"), height = 95, width = 6)
par(mfrow = c(38, 2))
...
pdf(paste0(plotDir, "RNAseq_meiocyte_Rep1_geneHexileProfiles_winSize20_landscape_4pages.pdf"), height = 10, width = 18)
par(mfrow = c(4, 6))
par(mar = c(2.1, 3.2, 2.1, 3.2))
par(mgp = c(2.25, 1, 0))
xplot <- seq(1, length(targetHexileCovProfiles[[1]][,1]), by = 1)
