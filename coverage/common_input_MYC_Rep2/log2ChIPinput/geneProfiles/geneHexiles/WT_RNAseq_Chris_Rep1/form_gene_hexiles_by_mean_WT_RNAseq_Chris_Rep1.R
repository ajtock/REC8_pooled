...
covDir <- "/home/meiosis/ajt200/analysis/170918_Chris_RNAseq_Col_kss/fastq_pooled/WT/coverage/"

RNAseq_norm <- read.table(paste0(covDir,
                                 "WT_RNAseq_Chris_Rep1_norm_allchrs_coverage_coord_tab.bed"))
allCovGenes <- NULL
for(i in 1:5) {
  print(i)
...
write.table(allCovGenes,
            file = paste0(hexDir,
                          "WT_RNAseq_Chris_Rep1_geneMeanCov.txt"))
...
for(j in 1:length(hex)) {
  ...
  if(j <= 6) {    
    ...
    write.table(hexjGenes,
                file = paste0(hexDir,
                              "hexile_", j,
                              "_WT_RNAseq_Chris_Rep1_geneCov.txt"))
  }
}
...
write.table(hexDat,
            file = paste0(hexDir,
                          "WT_RNAseq_Chris_Rep1_geneCov_hexile_stats.txt"),
            row.names = F)
