...
# Profile mean coverage around gene exons and random loci

# Usage via Condor submission system on node7:
# csmit -m 20G -c 1 "Rscript exon_Profiles_commandArgs.R /home/meiosis/ajt200/analysis/REC8_pooled/coverage/common_input_MYC_Rep2/log2ChIPinput/log2_REC8_HA_Rep1_ChIP_REC8_MYC_Rep2_input_norm_allchrs_coverage_coord_tab.bed REC8_HA_Rep1"
...
matDir <- "/home/meiosis/ajt200/analysis/REC8_pooled/coverage/common_input_MYC_Rep2/log2ChIPinput/geneProfiles/exons/analysis_01/matrices/"
...
# size distribution as exonsGR
ranLocGR <- randomizeRegions(exonsGR,
                             genome = genome,
                             per.chromosome = TRUE,
                             allow.overlaps = TRUE)
...
outDFCM <- list(list(paste0(matDir, libName,
                            "_norm_cov_exons_mat1_smoothed_target_and_flank_dataframe_colMeans.txt"),
...
covMatrix(signal = gr, target = exonsGR, ranLoc = ranLocGR,
          targetSize = mean(width(exonsGR)), flankSize = 1000, winSize = 20,
          outDFCM = outDFCM, x = 1)
