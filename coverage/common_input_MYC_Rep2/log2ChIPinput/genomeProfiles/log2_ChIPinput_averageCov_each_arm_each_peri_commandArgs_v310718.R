...
# csmit -m 20G -c 1 "./log2_ChIPinput_averageCov_each_arm_each_peri_commandArgs_v310718.R REC8_HA_Rep1 kss_REC8_HA_Rep1"
...
for(i in 1:5) {
  wt_chrCov_armL <- sum(wt_covDat[wt_covDat$V2 < pericenStart[i],]$V4)/(pericenStart[i]-1)
  wt_chrCov_armL_all <- c(wt_chrCov_armL_all, wt_chrCov_armL)
  wt_chrCov_armR <- sum(wt_covDat[wt_covDat$V2 > pericenEnd[i],]$V4)/(chrLens[i]-pericenEnd[i])
  wt_chrCov_armR_all <- c(wt_chrCov_armR_all, wt_chrCov_armR)
  wt_chrCov_peri <- sum(wt_covDat[wt_covDat$V2 >= pericenStart[i] & wt_covDat$V2 <= pericenEnd[i],]$V4)/(pericenEnd[i]-pericenStart[i]+1)
  wt_chrCov_peri_all <- c(wt_chrCov_peri_all, wt_chrCov_peri)

  mutant_chrCov_armL <- sum(mutant_covDat[mutant_covDat$V2 < pericenStart[i],]$V4)/(pericenStart[i]-1)
  mutant_chrCov_armL_all <- c(mutant_chrCov_armL_all, mutant_chrCov_armL)
  mutant_chrCov_armR <- sum(mutant_covDat[mutant_covDat$V2 > pericenEnd[i],]$V4)/(chrLens[i]-pericenEnd[i])
  mutant_chrCov_armR_all <- c(mutant_chrCov_armR_all, mutant_chrCov_armR)
  mutant_chrCov_peri <- sum(mutant_covDat[mutant_covDat$V2 >= pericenStart[i] & mutant_covDat$V2 <= pericenEnd[i],]$V4)/(pericenEnd[i]-pericenStart[i]+1)
