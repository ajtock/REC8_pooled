...
# Load concatenated log2-transformed coverage datasets
log2trans_list <- lapply(seq_along(ChIPnamesCombined), function(k) {
  read.table(file = paste0(outDirCombined[k], "log2_", ChIPnamesCombined[k], "_genome_norm_coverage_", winNames[1], ".txt"))
})
...
# Load concatenated gene dataset
GeneDat <- read.table(file = paste0(inDirFeatures, "gene_density_genome_", winNames[1], ".txt"))

# Load concatenated TE dataset
TEDat <- read.table(file = paste0(inDirFeatures, "TE_density_genome_", winNames[1], ".txt"))

# Load concatenated CO density datasets
CODat <- read.table(file = paste0(inDirFeatures, "WTCO_density_genome_", winNames[1], ".txt"))

# Load concatenated AT and GC content datasets
ATcontent <- read.table(file = paste0(inDirATGC, "AT_content_genome_", winNames[1], ".txt"))
GCcontent <- read.table(file = paste0(inDirATGC, "GC_content_genome_", winNames[1], ".txt"))

# Load concatenated DNA meth datasets
methDat <- read.table(file = paste0(inDirFeatures, "wtMeth_genome_", winNames[1], ".txt"))


# Create matrix of values within 10-kb windows
matrix10kbtmp <- sapply(seq_along(log2trans_list), function(x) cbind(log2trans_list[[x]][,2]))
matrix10kb <- cbind(log2trans_list[[1]][,1], matrix10kbtmp, GeneDat[,2], TEDat[,2], CODat[,2], ATcontent[,2], GCcontent[,2], methDat[,2:5])
colnames(matrix10kb) <- c("Windows", namesCombined, "Genes", "TEs", "Crossovers", "%AT", "%GC", "CG_meth", "CHG_meth", "CHH_meth", "DNA_meth_avg")
write.table(matrix10kb, file = paste0(outDir1, "REC8_matrix10kb_unsmoothed.txt"))
