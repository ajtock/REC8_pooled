...
factor_norm <- read.table(paste0(covDir,
                                 "log2_REC8_HA_Rep2_ChIP_REC8_MYC_Rep2_input_norm_allchrs_coverage_coord_tab_Chr4.bed"),
...
write.table(chrTargets,
            file = paste0(inDir, "Chr4_kb_per_uM_vs_mean_REC8_HA_Rep2_sum_REC8_HA_Rep2.txt"),
            sep = "\t", quote = F, row.names = F)

pdf(paste0(plotDir,
           "Chr4_kb_per_uM_vs_mean_REC8_HA_Rep2_sum_REC8_HA_Rep2.pdf"), height = 4, width = 8)
...
plot(x = chrTargets$kb.uM,
     ...
     xlab = "kb/uM",
     ylab = "Mean log2-transformed REC8-HA Rep2")
plot(x = chrTargets$kb.uM,
     ...
     xlab = "kb/uM",
     ylab = "Total log2-transformed REC8-HA Rep2")
dev.off()
