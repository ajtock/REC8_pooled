...
# Usage:
# ./log2_ChIPinput_perwin_arm_peri_rho_corr_matrices_noZscore_ASY1_v_others_v031019.R 10kb
...
libNames <- c(
              ...
              "DMC1_V5_Rep1_ChIP_REC8_MYC_Rep1_input",
...
MTOPVIB_Rep1 <- lib_filt_winDF_list[[3]]
MTOPVIB_Rep2 <- lib_filt_winDF_list[[4]]
DMC1_Rep1 <- lib_filt_winDF_list[[5]]
SPO11_1_oligos <- lib_filt_winDF_list[[6]]
Crossovers <- filt_COs
## Euchromatin:
Genes <- filt_genes
H3K4me1 <- lib_filt_winDF_list[[7]]
H3K4me2 <- lib_filt_winDF_list[[8]]
H3K4me3 <- lib_filt_winDF_list[[9]]
H2AZ <- lib_filt_winDF_list[[10]]
H3K27me3 <- lib_filt_winDF_list[[11]]
## Heterochromatin:
TEs <- filt_TEs
DNAmeth <- filt_meth
H2AW <- lib_filt_winDF_list[[12]]
H3K9me2 <- lib_filt_winDF_list[[13]]
H3K27me1 <- lib_filt_winDF_list[[14]]
Nucleosomes <- lib_filt_winDF_list[[15]]

allDF <- data.frame(ASY1[,6], REC8_HA[,6], MTOPVIB_Rep1[,6], MTOPVIB_Rep2[,6], DMC1_Rep1[,6], SPO11_1_oligos[,6], Crossovers[,3],
...
colnames(allDF) <- c("ASY1", "REC8-HA", "MTOPVIB Rep1", "MTOPVIB Rep2", "DMC1", "SPO11-1", "Crossovers",
...
pdf(file = paste0(plotDir, "ASY1_Rep2_filtered_genomewide_correlation_matrix_", winName, "_colouronly_noZscore_v031019.pdf"),
                  height = 12, width = 11)
...
MTOPVIB_Rep1_armL <- NULL
MTOPVIB_Rep1_armR <- NULL
MTOPVIB_Rep1_arm <- NULL
MTOPVIB_Rep1_peri <- NULL

MTOPVIB_Rep2_armL <- NULL
MTOPVIB_Rep2_armR <- NULL
MTOPVIB_Rep2_arm <- NULL
MTOPVIB_Rep2_peri <- NULL
...
for(i in 1:5) {
  ...
  print(i)
  MTOPVIB_Rep1_armL_tmp <- MTOPVIB_Rep1[MTOPVIB_Rep1[,3] > sumchr[i] & MTOPVIB_Rep1[,3] < pericenStart[i] & MTOPVIB_Rep1[,3] < sumchr[i+1],]
  MTOPVIB_Rep1_armR_tmp <- MTOPVIB_Rep1[MTOPVIB_Rep1[,3] > pericenEnd[i] & MTOPVIB_Rep1[,3] <= sumchr[i+1],]
  MTOPVIB_Rep1_arm_tmp <- MTOPVIB_Rep1[(MTOPVIB_Rep1[,3] > sumchr[i] & MTOPVIB_Rep1[,3] < pericenStart[i] & MTOPVIB_Rep1[,3] < sumchr[i+1]) |
                                       (MTOPVIB_Rep1[,3] > pericenEnd[i] & MTOPVIB_Rep1[,3] <= sumchr[i+1]),]
  MTOPVIB_Rep1_peri_tmp <- MTOPVIB_Rep1[MTOPVIB_Rep1[,3] >= pericenStart[i] & MTOPVIB_Rep1[,3] <= pericenEnd[i],]
  MTOPVIB_Rep1_peri <- rbind(MTOPVIB_Rep1_peri, MTOPVIB_Rep1_peri_tmp)
  MTOPVIB_Rep1_armL <- rbind(MTOPVIB_Rep1_armL, MTOPVIB_Rep1_armL_tmp)
  MTOPVIB_Rep1_armR <- rbind(MTOPVIB_Rep1_armR, MTOPVIB_Rep1_armR_tmp)
  MTOPVIB_Rep1_arm <- rbind(MTOPVIB_Rep1_arm, MTOPVIB_Rep1_arm_tmp)

  print(i)
  MTOPVIB_Rep2_armL_tmp <- MTOPVIB_Rep2[MTOPVIB_Rep2[,3] > sumchr[i] & MTOPVIB_Rep2[,3] < pericenStart[i] & MTOPVIB_Rep2[,3] < sumchr[i+1],]
  MTOPVIB_Rep2_armR_tmp <- MTOPVIB_Rep2[MTOPVIB_Rep2[,3] > pericenEnd[i] & MTOPVIB_Rep2[,3] <= sumchr[i+1],]
  MTOPVIB_Rep2_arm_tmp <- MTOPVIB_Rep2[(MTOPVIB_Rep2[,3] > sumchr[i] & MTOPVIB_Rep2[,3] < pericenStart[i] & MTOPVIB_Rep2[,3] < sumchr[i+1]) |
                                       (MTOPVIB_Rep2[,3] > pericenEnd[i] & MTOPVIB_Rep2[,3] <= sumchr[i+1]),]
  MTOPVIB_Rep2_peri_tmp <- MTOPVIB_Rep2[MTOPVIB_Rep2[,3] >= pericenStart[i] & MTOPVIB_Rep2[,3] <= pericenEnd[i],]
  MTOPVIB_Rep2_peri <- rbind(MTOPVIB_Rep2_peri, MTOPVIB_Rep2_peri_tmp)
  MTOPVIB_Rep2_armL <- rbind(MTOPVIB_Rep2_armL, MTOPVIB_Rep2_armL_tmp)
  MTOPVIB_Rep2_armR <- rbind(MTOPVIB_Rep2_armR, MTOPVIB_Rep2_armR_tmp)
  MTOPVIB_Rep2_arm <- rbind(MTOPVIB_Rep2_arm, MTOPVIB_Rep2_arm_tmp)

  print(i)
  DMC1_Rep1_armL_tmp <- DMC1_Rep1[DMC1_Rep1[,3] > sumchr[i] & DMC1_Rep1[,3] < pericenStart[i] & DMC1_Rep1[,3] < sumchr[i+1],]
  DMC1_Rep1_armR_tmp <- DMC1_Rep1[DMC1_Rep1[,3] > pericenEnd[i] & DMC1_Rep1[,3] <= sumchr[i+1],]
  DMC1_Rep1_arm_tmp <- DMC1_Rep1[(DMC1_Rep1[,3] > sumchr[i] & DMC1_Rep1[,3] < pericenStart[i] & DMC1_Rep1[,3] < sumchr[i+1]) |
                                 (DMC1_Rep1[,3] > pericenEnd[i] & DMC1_Rep1[,3] <= sumchr[i+1]),]
  DMC1_Rep1_peri_tmp <- DMC1_Rep1[DMC1_Rep1[,3] >= pericenStart[i] & DMC1_Rep1[,3] <= pericenEnd[i],]
  DMC1_Rep1_peri <- rbind(DMC1_Rep1_peri, DMC1_Rep1_peri_tmp)
  DMC1_Rep1_armL <- rbind(DMC1_Rep1_armL, DMC1_Rep1_armL_tmp)
  DMC1_Rep1_armR <- rbind(DMC1_Rep1_armR, DMC1_Rep1_armR_tmp)
  DMC1_Rep1_arm <- rbind(DMC1_Rep1_arm, DMC1_Rep1_arm_tmp)
...
allDF_arm <- data.frame(ASY1_arm[,6], REC8_HA_arm[,6], MTOPVIB_Rep1_arm[,6], MTOPVIB_Rep2_arm[,6], DMC1_Rep1_arm[,6], SPO11_1_oligos_arm[,6], Crossovers_arm[,3],
...
colnames(allDF_arm) <- c("ASY1", "REC8-HA", "MTOPVIB Rep1", "MTOPVIB Rep2", "DMC1", "SPO11-1", "Crossovers",
                         "Genes", "H3K4me1", "H3K4me2", "H3K4me3", "H2A.Z", "H3K27me3",
                         "TEs", "mCG", "mCHG", "mCHH", "H2A.W", "H3K9me2", "H3K27me1", "Nucleosomes")
allDF_arm_corMat <- cor(allDF_arm, method = "spearman", use = "pairwise.complete.obs")
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "ASY1_Rep2_filtered_arms_correlation_matrix_", winName, "_colouronly_noZscore_v031019.pdf"),
                  height = 12, width = 11)
...
allDF_peri <- data.frame(ASY1_peri[,6], REC8_HA_peri[,6], MTOPVIB_Rep1_peri[,6], MTOPVIB_Rep2_peri[,6], DMC1_Rep1_peri[,6], SPO11_1_oligos_peri[,6], Crossovers_peri[,3],
...
colnames(allDF_peri) <- c("ASY1", "REC8-HA", "MTOPVIB Rep1", "MTOPVIB Rep2", "DMC1", "SPO11-1", "Crossovers",
                          "Genes", "H3K4me1", "H3K4me2", "H3K4me3", "H2A.Z", "H3K27me3",
                          "TEs", "mCG", "mCHG", "mCHH", "H2A.W", "H3K9me2", "H3K27me1", "Nucleosomes")
allDF_peri_corMat <- cor(allDF_peri, method = "spearman", use = "pairwise.complete.obs")
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "ASY1_Rep2_filtered_pericentromeres_correlation_matrix_", winName, "_colouronly_noZscore_v031019.pdf"),
                  height = 12, width = 11)
...
pdf(file = paste0(plotDir, "ASY1_Rep2_filtered_genomewide_arms_pericentromeres_correlation_matrix_", winName, "_colouronly_noZscore_v031019.pdf"),
                  height = 25.5, width = 8)
par(mfcol = c(3, 1))
corrplot(allDF_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         addgrid.col = "white", addCoef.col = "grey90", mar = c(0,0,1,0), tl.cex = 1.5, cl.cex = 1.5, number.cex = 1.0,
