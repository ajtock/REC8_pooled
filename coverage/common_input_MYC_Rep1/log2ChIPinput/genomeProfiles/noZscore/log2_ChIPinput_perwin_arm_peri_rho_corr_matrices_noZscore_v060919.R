...
# Usage:
# ./log2_ChIPinput_perwin_arm_peri_rho_corr_matrices_noZscore_v060919.R 10kb
...
REC8_HA <- lib_filt_winDF_list[[1]]
## Recombination:
SPO11_1_oligos <- lib_filt_winDF_list[[2]]
Crossovers <- filt_COs
## Euchromatin:
Genes <- filt_genes
H3K4me1 <- lib_filt_winDF_list[[3]]
H3K4me2 <- lib_filt_winDF_list[[4]]
H3K4me3 <- lib_filt_winDF_list[[5]]
H2AZ <- lib_filt_winDF_list[[6]]
H3K27me3 <- lib_filt_winDF_list[[7]]
## Heterochromatin:
TEs <- filt_TEs
DNAmeth <- filt_meth
H2AW <- lib_filt_winDF_list[[8]]
H3K9me2 <- lib_filt_winDF_list[[9]]
H3K27me1 <- lib_filt_winDF_list[[10]]
Nucleosomes <- lib_filt_winDF_list[[11]]

allDF <- data.frame(REC8_HA[,6], SPO11_1_oligos[,6], Crossovers[,3],
...
pdf(file = paste0(plotDir, "REC8_HA_Rep2_filtered_genomewide_correlation_matrix_", winName, "_colouronly_noZscore_v060919.pdf"),
                  height = 12, width = 11)
...
allDF_arm <- data.frame(REC8_HA_arm[,6], SPO11_1_oligos_arm[,6], Crossovers_arm[,3],
...
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "REC8_HA_Rep2_filtered_arms_correlation_matrix_", winName, "_colouronly_noZscore_v060919.pdf"),
                  height = 12, width = 11)
...
allDF_peri <- data.frame(REC8_HA_peri[,6], SPO11_1_oligos_peri[,6], Crossovers_peri[,3],
...
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "REC8_HA_Rep2_filtered_pericentromeres_correlation_matrix_", winName, "_colouronly_noZscore_v060919.pdf"),
                  height = 12, width = 11)
...
pdf(file = paste0(plotDir, "REC8_HA_Rep2_filtered_genomewide_arms_pericentromeres_correlation_matrix_", winName, "_colouronly_noZscore_v060919.pdf"),
                  height = 25.5, width = 8)
par(mfcol = c(3, 1))
corrplot(allDF_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         addgrid.col = "white", addCoef.col = "grey90", mar = c(0,0,1,0), tl.cex = 1.5, cl.cex = 1.5, number.cex = 1.0,
