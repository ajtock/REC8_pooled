...
# REC8-HA Rep2 (red) vs:
# REC8-Myc Rep1 (goldenrod1)
# DNA methylation ("navy", "blue", "deepskyblue1")

# Usage:
# Rscript ./log2_ChIPinput_genomewideZscore_perwin_PLOTONLY_v150419.R 10kb
...
# All
inDirAll <- c(inDir1, inDir2)
prefixAll <- c(prefix1, prefix2)
nameAll <- c(name1, name2)
colourAll <- c(colour1, colour2)
...
REC8vsDNAmethSepGenomePlot <- function(xplot1,
                                       ...
                                       dat2,
                                       legendNames, plotColours) {
  plot(xplot2, dat2[,3], type = "l", lwd = 2, col = plotColours[3],
  ...
  lines(xplot2, dat2[,4], type = "l", lwd = 2, col = plotColours[4])
  lines(xplot2, dat2[,5], type = "l", lwd = 2, col = plotColours[5])
  axis(side = 4, at = pretty(c(dat2[,3], dat2[,4], dat2[,5])), lwd.tick = 2)
  par(new = T)
  plot(xplot1, dat1A, type = "l", lwd = 2, col = plotColours[1],
       ylim = c(min(dat1A, dat1B), max(dat1A, dat1B)),
       xlab = "", ylab = "",
       xaxt = "n", yaxt = "n")
  lines(xplot1, dat1B, type = "l", lwd = 2, col = plotColours[2])
  axis(side = 1, lwd.tick = 2,
       at = c(0, 2e7, 4e7, 6e7, 8e7, 1e8, 1.2e8),
       labels = c("0", "20", "40", "60", "80", "100", "120"), cex.axis = 1.25)
  mtext(side = 1, line = 2.30, cex = 2,
...
#
pdf(paste0(plotDir, "genomewideZscore_log2_", REC8HAprefix, "_", winName, "_genomeProfiles_v150419.pdf"),
    height = 9, width = 12)
par(mfcol = c(2, 1))
...
dat1to4GenomePlot(xplot = filt_REC8log2trans_list[[1]]$cumWindows,
                  ...
                  dat3  = filt_log2trans_list[[1]]$filt_ZscoreLog2cov,
                  dat4  = filt_log2trans_list[[2]]$filt_ZscoreLog2cov,
                  legendNames = c(nameREC8, nameAll),
                  plotColours = c(colourREC8, colourAll))
REC8vsDNAmethSepGenomePlot(xplot1 = filt_REC8log2trans_list[[1]]$cumWindows,
                           ...
                           dat1B = filt_REC8log2trans_list[[2]]$filt_ZscoreLog2cov,
                           dat2 = filt_meth,
                           legendNames = c(nameREC8, "mCG", "mCHG", "mCHH"),
                           plotColours = c(colourREC8, "navy", "blue", "deepskyblue1"))
dev.off()
