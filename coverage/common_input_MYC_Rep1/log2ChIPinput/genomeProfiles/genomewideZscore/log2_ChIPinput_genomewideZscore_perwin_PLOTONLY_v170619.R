...
# Usage:
# Rscript ./log2_ChIPinput_genomewideZscore_perwin_PLOTONLY_v170619.R 10kb
...
REC8GenomePlot <- function(xplot,
  ...
  mtext(side = 2, line = 3.00, cex = 1.3, text = expression("Log"[2]*"(ChIP/input)"))
  abline(v = sumchr, lty = 1, lwd = 2)
  abline(v = centromeres, lty = 5, lwd = 2)
  box(lwd = 2)
  legend("topleft",
...
#
pdf(paste0(plotDir, "genomewideZscore_log2_", REC8HAprefix, "_", winName, "_genomeProfiles_v170619.pdf"),
    height = 22.5, width = 12)
par(mfcol = c(5, 1))
par(mar = c(6.1, 6.1, 2.1, 6.1))
par(mgp = c(3, 1.5, 0))
dat1to2GenomePlot(xplot = filt_REC8log2trans_list[[1]]$cumWindows,
                  dat1 = filt_REC8log2trans_list[[1]]$filt_ZscoreLog2cov,
                  dat2 = filt_log2trans_list[[1]]$filt_ZscoreLog2cov,
                  legendNames = c(nameREC8, nameAll[1]),
                  plotColours = c(colourREC8, colourAll[1]))
dat1to2vFeatureGenomePlot(xplot = filt_REC8log2trans_list[[1]]$cumWindows,
                          dat1 = filt_REC8log2trans_list[[1]]$filt_ZscoreLog2cov,
                          dat2 = filt_log2trans_list[[2]]$filt_ZscoreLog2cov,
                          feature1 = filt_TEs$filt_TEs,
                          legendNames = c(nameREC8, nameAll[2]),
                          legendPos = "topleft",
                          feature1Lab = "TEs",
                          plotColours = c(colourREC8, colourAll[2]),
                          feature1Colour = "darkgreen")
...
dat1to2vFeatureGenomePlot(xplot = filt_REC8log2trans_list[[1]]$cumWindows,
                          ...
                          feature1 = filt_genes$filt_genes,
                          legendNames = c(nameREC8, nameAll[3]),
                          legendPos = "bottomleft",
                          feature1Lab = "Genes",
                          plotColours = c(colourREC8, colourAll[3]),
                          feature1Colour = "goldenrod4")
dat1to2vFeatureGenomePlot(xplot = filt_REC8log2trans_list[[1]]$cumWindows,
                          dat1 = filt_REC8log2trans_list[[1]]$filt_ZscoreLog2cov,
                          dat2 = filt_log2trans_list[[4]]$filt_ZscoreLog2cov,
                          feature1 = filt_COs$filt_COs,
                          legendNames = c(nameREC8, nameAll[4]),
                          legendPos = "topleft",
                          feature1Lab = "Crossovers",
                          plotColours = c(colourREC8, colourAll[4]),
                          feature1Colour = "cyan")
dev.off()
