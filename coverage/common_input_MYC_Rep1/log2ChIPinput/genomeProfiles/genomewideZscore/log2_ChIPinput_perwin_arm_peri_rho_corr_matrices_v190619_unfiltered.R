...
# Usage:
# ./log2_ChIPinput_perwin_arm_peri_rho_corr_matrices_v190619_unfiltered.R 10kb
...
REC8log2trans_list <- mclapply(seq_along(nameREC8), function(x) {
  read.table(paste0(inDirREC8[x], "log2_", prefixREC8[x],
...
log2trans_list <- mclapply(seq_along(nameAll), function(x) {
  read.table(paste0(inDirAll[x], "log2_", prefixAll[x],
                    "_genome_norm_coverage_Zscore_", winName, ".txt"))
...
# TEs
TEs <- read.table(paste0(inDirFeatures, "TE_frequency_genome_",
                              winName, ".txt"))
# genes
genes <- read.table(paste0(inDirFeatures, "gene_frequency_genome_",
                                winName, ".txt"))
# COs
COs <- read.table(paste0(inDirFeatures, "CO_frequency_genome_",
                              winName, ".txt"))
# DNA methylation in 200-kb windows
meth <- read.table(paste0(inDirFeatures, "DNAmeth_GSM980986_WT_rep2_genome_",
                               winName, ".txt"))


# Create FILTERED, log2-transformed genome-wide Spearman's rho correlation matrix

REC8_HA <- REC8log2trans_list[[1]]
REC8_MYC <- REC8log2trans_list[[2]]
## Recombination:
SPO11_1_oligos <- log2trans_list[[10]]
Crossovers <- COs
## Euchromatin:
Genes <- genes
H3K4me1 <- log2trans_list[[5]]
H3K4me2 <- log2trans_list[[6]]
H3K4me3 <- log2trans_list[[7]]
H2AZ <- log2trans_list[[8]]
H3K27me3 <- log2trans_list[[9]]
## Heterochromatin:
TEs <- TEs
DNAmeth <- meth
H2AW <- log2trans_list[[1]]
H3K9me2 <- log2trans_list[[2]]
H3K27me1 <- log2trans_list[[3]]
Nucleosomes <- log2trans_list[[4]]

allDF <- data.frame(REC8_HA[,3], SPO11_1_oligos[,3], Crossovers[,3],
                    Genes[,3], H3K4me1[,3], H3K4me2[,3], H3K4me3[,3], H2AZ[,3], H3K27me3[,3],
                    TEs[,3], DNAmeth[,3], DNAmeth[,4], DNAmeth[,5], H2AW[,3], H3K9me2[,3], H3K27me1[,3], Nucleosomes[,3])
colnames(allDF) <- c("REC8-HA", "SPO11-1", "Crossovers",
...
pdf(file = paste0(plotDir, "REC8_HA_Rep2_unfiltered_genomewide_correlation_matrix_", winName, "_colouronly_genomewideZscore_v190619.pdf"),
                  height = 12, width = 11)
corrplot(allDF_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         ...
         title = paste0("Unfiltered genome-wide Spearman correlation matrix (10-kb windows)"))
dev.off()
...
allDF_arm <- data.frame(REC8_HA_arm[,3], SPO11_1_oligos_arm[,3], Crossovers_arm[,3],
                        Genes_arm[,3], H3K4me1_arm[,3], H3K4me2_arm[,3], H3K4me3_arm[,3], H2AZ_arm[,3], H3K27me3_arm[,3],
                        TEs_arm[,3], DNAmeth_arm[,3], DNAmeth_arm[,4], DNAmeth_arm[,5], H2AW_arm[,3], H3K9me2_arm[,3], H3K27me1_arm[,3], Nucleosomes_arm[,3])
colnames(allDF_arm) <- c("REC8-HA", "SPO11-1", "Crossovers",
                         "Genes", "H3K4me1", "H3K4me2", "H3K4me3", "H2A.Z", "H3K27me3",
                         "TEs", "mCG", "mCHG", "mCHH", "H2A.W", "H3K9me2", "H3K27me1", "Nucleosomes")
allDF_arm_corMat <- cor(allDF_arm, method = "spearman", use = "pairwise.complete.obs")
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "REC8_HA_Rep2_unfiltered_arms_correlation_matrix_", winName, "_colouronly_genomewideZscore_v190619.pdf"),
                  height = 12, width = 11)
corrplot(allDF_arm_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         addgrid.col = "white", addCoef.col = "grey90", mar = c(0,0,1,0), tl.cex = 1.5, cl.cex = 1.5, number.cex = 1.0,
         title = paste0("Unfiltered arms Spearman correlation matrix (10-kb windows)"))
dev.off()

allDF_peri <- data.frame(REC8_HA_peri[,3], SPO11_1_oligos_peri[,3], Crossovers_peri[,3],
                         Genes_peri[,3], H3K4me1_peri[,3], H3K4me2_peri[,3], H3K4me3_peri[,3], H2AZ_peri[,3], H3K27me3_peri[,3],
                         TEs_peri[,3], DNAmeth_peri[,3], DNAmeth_peri[,4], DNAmeth_peri[,5], H2AW_peri[,3], H3K9me2_peri[,3], H3K27me1_peri[,3], Nucleosomes_peri[,3])
colnames(allDF_peri) <- c("REC8-HA", "SPO11-1", "Crossovers",
                          "Genes", "H3K4me1", "H3K4me2", "H3K4me3", "H2A.Z", "H3K27me3",
                          "TEs", "mCG", "mCHG", "mCHH", "H2A.W", "H3K9me2", "H3K27me1", "Nucleosomes")
allDF_peri_corMat <- cor(allDF_peri, method = "spearman", use = "pairwise.complete.obs")
col1 <- colorRampPalette(c("blue", "white", "red"))
pdf(file = paste0(plotDir, "REC8_HA_Rep2_unfiltered_pericentromeres_correlation_matrix_", winName, "_colouronly_genomewideZscore_v190619.pdf"),
                  height = 12, width = 11)
corrplot(allDF_peri_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         addgrid.col = "white", addCoef.col = "grey90", mar = c(0,0,1,0), tl.cex = 1.5, cl.cex = 1.5, number.cex = 1.0,
         title = paste0("Unfiltered pericentromeres Spearman correlation matrix (10-kb windows)"))      
dev.off()


pdf(file = paste0(plotDir, "REC8_HA_Rep2_unfiltered_genomewide_arms_pericentromeres_correlation_matrix_", winName, "_colouronly_genomewideZscore_v190619.pdf"),
                  height = 25.5, width = 8)
par(mfcol = c(3, 1))
corrplot(allDF_corMat, method = "color", type = "upper", col = col1(20), tl.col = "black",
         addgrid.col = "white", addCoef.col = "grey90", mar = c(0,0,1,0), tl.cex = 1.5, cl.cex = 1.5, number.cex = 1.0,
