...
# Usage:
# /applications/R/R-3.4.0/bin/Rscript features_heatmap_sorted.R REC8_HA_Rep1 genes 2000 2kb '2 kb' 20 20bp bodies
...
foreach(x = 1:length(ChIPseqNames)) %dopar% {
  ChIPseq_col_fun <- colorRamp2(c(-0.5, 1),
                                c("white", ChIPseqColours[x]))
...
# Plot average profile with SD as ribbon
# Example for one dataset below to be adapted
# to loop through all datasets

# Transpose matrix and convert to dataframe
# in which first column is window name

wideDF <- data.frame(window = colnames(mats[[1]]),
                     t(mats[[1]]))
tidyDF <- gather(data  = wideDF,
                 key   = feature,
                 value = coverage,
                 -window)
tidyDF$window <- factor(tidyDF$window,
                        levels = as.character(wideDF$window))

summaryDF <- data.frame(window = as.character(wideDF$window),
                        n      = tapply(X     = tidyDF$coverage,
                                        INDEX = tidyDF$window,
                                        FUN   = length),
                        mean   = tapply(X     = tidyDF$coverage,
                                        INDEX = tidyDF$window,
                                        FUN   = mean),
                        sd     = tapply(X     = tidyDF$coverage,
                                        INDEX = tidyDF$window,
                                        FUN   = sd))
summaryDF$sem <- summaryDF$sd/sqrt(summaryDF$n-1)
summaryDF$CI_lower <- summaryDF$mean -
                        qt(0.975, df = summaryDF$n-1)*summaryDF$sem
summaryDF$CI_upper <- summaryDF$mean +
                        qt(0.975, df = summaryDF$n-1)*summaryDF$sem


ggObj <- ggplot(summaryDF,
                aes(x = 1:310, y = mean)) +
  geom_line(colour = "red", size = 1, alpha = 0.8) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper),
              fill = "red", alpha = 0.4) +
  theme_light(base_size = 16)
ggsave("./REC8_around_genes.pdf", plot = ggObj)
