...
# Usage:
# /applications/R/R-3.5.0/bin/Rscript features_avgProfileRibbon_Corr.R HypoCHG_DMRs_kss 'HypoCHG DMRs' 2000 2kb '2 kb' 20 20bp 'REC8_HA_Rep2,REC8_MYC_Rep1' 'REC8-HA,REC8-Myc' 'red,blue' '0.02,0.94'
...
#binSize <- 20
#binName <- "20bp"
#libNames <- unlist(strsplit("REC8_HA_Rep2,REC8_MYC_Rep1",
#                            split = ","))
#libNamesPlot <- unlist(strsplit("REC8-HA,REC8-Myc",
#                                split = ","))
#colours <- unlist(strsplit("red,blue",
#                           split = ","))
...
legendLab1 <- grobTree(textGrob(bquote(.(libNamesPlot[1])),
                                ...
                                gp = gpar(col = colours[1], fontsize = 22))) 
legendLab2 <- grobTree(textGrob(bquote(.(libNamesPlot[2])),
                                x = legendPos[1], y = legendPos[2]-0.1, just = "left",
                                gp = gpar(col = colours[2], fontsize = 22)))
...
ggObj1 <- ggplot(data = summaryDFfeature,
  ...
  ggtitle(bquote(.(featureNamePlot) ~ italic("r"[s]) ~ "=" ~
                 .(round(cor(summaryDFfeature_list[[1]]$mean,
                             summaryDFfeature_list[[2]]$mean,
                             method = "spearman"),
                         digits = 2))))
## ranLoc
ggObj2 <- NULL
ggObj2 <- ggplot(data = summaryDFranLoc,
  ...
  theme(
        ...
        plot.margin = unit(c(0.3,0.9,0.0,0.3), "cm"), 
        plot.title = element_text(hjust = 0.5, size = 30)) +
  ggtitle(bquote("Random loci" ~ italic("r"[s]) ~ "=" ~
                 .(round(cor(summaryDFranLoc_list[[1]]$mean,
                             summaryDFranLoc_list[[2]]$mean,
                             method = "spearman"),
                         digits = 2))))
