...
# Profile mean coverage around DNA TEs and random loci

# Usage via Condor submission system on node7:
# csmit -m 20G -c 1 "./TE_DNAfams_Profiles_commandArgs.R 2000 2kb 20 /home/ajt200/analysis/REC8_pooled/coverage/common_input_MYC_Rep1/log2ChIPinput/log2_REC8_HA_Rep1_ChIP_REC8_MYC_Rep1_input_norm_allchrs_coverage_coord_tab.bed REC8_HA_Rep1"
...
for(h in seq_along(DNAfamNames)) {
  ...
  outDF <- list(paste0(matDir, libName,
                       "_norm_cov_", DNAfamNames[h], "_TEs_smoothed_target_and_", flankName, "_flank_dataframe.txt"),
                paste0(matDir, libName,
                       "_norm_cov_", DNAfamNames[h], "_ranLoc_smoothed_target_and_", flankName, "_flank_dataframe.txt"))
  outDFcolMeans <- list(paste0(matDir, libName,
                               "_norm_cov_", DNAfamNames[h], "_TEs_smoothed_target_and_", flankName, "_flank_dataframe_colMeans.txt"),
                        paste0(matDir, libName,
                               "_norm_cov_", DNAfamNames[h], "_ranLoc_smoothed_target_and_", flankName, "_flank_dataframe_colMeans.txt"))
