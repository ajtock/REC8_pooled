...
# Usage:
# /applications/R/R-3.4.0/bin/Rscript features_heatmap_sorted.R REC8_HA_Rep2 enspm 2000 2kb '2 kb' 20 20bp bodies
...
regionPlotDir <- paste0(plotDir, region,
                        "_by_", libName, "/")
featurePlotDir <- paste0(regionPlotDir, featureName, "/")
system(paste0("[ -d ", plotDir, " ] || mkdir ", plotDir))
system(paste0("[ -d ", regionPlotDir, " ] || mkdir ", regionPlotDir))
system(paste0("[ -d ", featurePlotDir, " ] || mkdir ", featurePlotDir))
plotDir <- featurePlotDir
...
foreach(x = 1:length(ChIPseqNames)) %dopar% {
  ...
  pdf(paste0(plotDir, ChIPseqNames[x], "_around_", featureName,
             "_TEs_heatmap_ordered_by_", libName, "_in_", region, ".pdf"),
      width = 5, height = 10)
