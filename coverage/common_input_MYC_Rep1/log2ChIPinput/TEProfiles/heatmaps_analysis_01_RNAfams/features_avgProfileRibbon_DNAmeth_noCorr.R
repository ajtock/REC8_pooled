...
# Usage:
# /applications/R/R-3.5.0/bin/Rscript features_avgProfileRibbon_DNAmeth_noCorr.R RNA_TEs 'RNA TEs' 2000 2kb '2 kb' 20 20bp 'mCG,mCHG,mCHH' 'mCG,mCHG,mCHH' 'darkorange3,orange,yellow3' '0.02,0.70'
...
#binSize <- 20
#binName <- "20bp"
#libNames <- unlist(strsplit("mCG,mCHG,mCHH",
#                            split = ","))
#libNamesPlot <- unlist(strsplit("mCG,mCHG,mCHH",
#                                split = ","))
#colours <- unlist(strsplit("darkorange3,orange,yellow3",
#                           split = ","))
...
featureMats <- mclapply(seq_along(libNames), function(x) {
  as.matrix(read.table(paste0(matDir,
                              libNames[x],                            
                              "_norm_cov_rna_TEs_smoothed_target_and_",
...
ranLocMats <- mclapply(seq_along(libNames), function(x) {
  as.matrix(read.table(paste0(matDir,
                              libNames[x],                            
                              "_norm_cov_rna_ranLoc_smoothed_target_and_",
                              flankName, "_flank_dataframe.txt"),
                       header = T))
}, mc.cores = length(libNames))
